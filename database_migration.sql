-- TalentConnect - Database Migration for Missing Tables and Columns
...
CREATE TABLE IF NOT EXISTS token_transactions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    transaction_type VARCHAR(20) NOT NULL, -- 'earn' or 'spend'
...
CREATE TABLE IF NOT EXISTS skill_verification_quizzes (
    ...
    passing_score INTEGER NOT NULL, -- Minimum score to pass
...
CREATE TABLE IF NOT EXISTS mentor_matches (
    ...
    skill_name VARCHAR(255) NOT NULL,
    similarity_score DECIMAL(5,4), -- Matching algorithm score
    status VARCHAR(20) DEFAULT 'suggested', -- 'suggested', 'contacted', 'accepted'
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS learning_roadmaps (
    ...
    roadmap_data JSONB NOT NULL, -- AI-generated roadmap with steps
...
-- All new tables follow the same UUID pattern as existing tables
-- Indexes added for frequently queried columns
-- Sample quizzes provided for Python and React
-- Add more quizzes as needed via the admin panel
